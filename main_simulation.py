...
class ExtendedStateObserver:
    """
    扩张状态观测器（ESO）用于在线扰动估计

    该观测器将扰动状态增广到系统状态中，实现对未知扰动和模型不确定性的实时估计。
...
class LyapunovStabilityAnalyzer:
    """
    Lyapunov函数用于稳定性分析和约束生成

    Lyapunov函数设计为同时捕获跟踪性能和摆动抑制目标：
    ...
    def compute_terminal_constraint(self, e_p_N: np.ndarray, e_v_N: np.ndarray,
        ...
        该约束通过要求预测终端Lyapunov值满足：
        V(N) <= exp(-lambda * N * dt) * V(0)
        来保证递归可行性和稳定性。

        返回:
            constraint_value: 可行性要求该值 <= 0
        """
...
class AdaptiveWeightingStrategy:
    ...
    自适应权重根据实时状态误差动态调整，智能平衡轨迹跟踪和摆动抑制：
    ...
    这确保了当误差较大时进行激进跟踪，当系统接近平衡时进行平滑控制。
    """
    ...
    def update_weights(self, e_p: np.ndarray, alpha: float, beta: float,
        ...
        参数:
            e_p: 位置跟踪误差
            alpha, beta: 摆角
            du_prev: 上一步控制增量（用于自适应S）

        返回:
            Q, R, S, W_swing: 更新后的权重矩阵
        """
...
class ConstrainedAdaptiveMPC:
    ...
    def solve_mpc(self, x_current: np.ndarray, x_ref_trajectory: np.ndarray,
        ...
        参数:
            x_current: 当前状态
            x_ref_trajectory: 预测时域上的参考轨迹
            u_prev: 上一步控制输入

        返回:
            u_optimal: 当前时刻最优控制输入
            x_predicted: 预测轨迹
            solve_time: 计算时间
            feasible: 解是否可行
        """
...
class ResultsAnalyzer:
    ...
    def plot_comprehensive_results(results: Dict, save_path: str = None):
        ...
        if 'controller_metrics' in results:
            ...
            # Ensure length matches time
            n_points = min(len(time), len(kappa_e))
            time_ctrl = time[:n_points]
            kappa_e = kappa_e[:n_points]
            kappa_theta = kappa_theta[:n_points]
