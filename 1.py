...
# Step 1: Draw 3D coordinate system
O = np.array([0.5, 0.5, 0])
axis_len = 1.8

# X-axis (red - softer tone)
_arrow3D(ax, O[0], O[1], O[2], axis_len, 0, 0,
         mutation_scale=25, lw=3.5, arrowstyle='-|>', color='#C85A54')
ax.text(O[0]+axis_len+0.25, O[1], O[2], r'$x$', fontsize=18, fontweight='bold', color='#C85A54')

# Y-axis (green - softer tone)
_arrow3D(ax, O[0], O[1], O[2], 0, axis_len, 0,
         mutation_scale=25, lw=3.5, arrowstyle='-|>', color='#5A9367')
ax.text(O[0], O[1]+axis_len+0.25, O[2], r'$y$', fontsize=18, fontweight='bold', color='#5A9367')

# Z-axis (blue - softer tone)
_arrow3D(ax, O[0], O[1], O[2], 0, 0, axis_len,
         mutation_scale=25, lw=3.5, arrowstyle='-|>', color='#5B7C99')
ax.text(O[0]-0.15, O[1], O[2]+axis_len+0.25, r'$z$', fontsize=18, fontweight='bold', color='#5B7C99')

# Origin marker
ax.scatter(O[0], O[1], O[2], color='black', s=80, zorder=10)
ax.text(O[0]-0.25, O[1]-0.25, O[2]-0.15, r'$O$', fontsize=14, fontweight='bold')

# Coordinate system label
ax.text(O[0]+0.3, O[1]-0.5, O[2]-0.35, 'Inertial frame {O, x, y, z}',
        fontsize=11, style='italic', color='#333333')

# Step 2: Draw 3D ground plane with grid
ground_z = 0.2
ground_x_start = 1.5
ground_y_start = 1.5
ground_size = 3.2

grid_x = np.linspace(ground_x_start, ground_x_start + ground_size, 5)
grid_y = np.linspace(ground_y_start, ground_y_start + ground_size, 5)

for gx in grid_x:
    ax.plot([gx, gx], [ground_y_start, ground_y_start + ground_size],
            [ground_z, ground_z],
            color='#999999', linewidth=0.8, alpha=0.3, linestyle='--')
for gy in grid_y:
    ax.plot([ground_x_start, ground_x_start + ground_size], [gy, gy],
            [ground_z, ground_z],
            color='#999999', linewidth=0.8, alpha=0.3, linestyle='--')

ground_corners = np.array([
    [ground_x_start, ground_y_start, ground_z],
    [ground_x_start + ground_size, ground_y_start, ground_z],
    [ground_x_start + ground_size, ground_y_start + ground_size, ground_z],
    [ground_x_start, ground_y_start + ground_size, ground_z]
])
ground_plane = Poly3DCollection([ground_corners], alpha=0.08,
                                facecolor='#CCCCCC', edgecolor='#999999', linewidth=1.5)
ax.add_collection3d(ground_plane)

# Step 3: Draw 3D UAV body and center of mass
uav_x, uav_y, uav_z = 4.5, 3.5, 5.5
body_w, body_d, body_h = 1.0, 0.8, 0.3
...
uav_vertices = draw_box(ax, [uav_x, uav_y, uav_z], body_w, body_d, body_h,
                        color='#D3D3D3', alpha=0.9)

rotor_r = 0.22
arm_len = 0.5
rotor_positions = [
    [uav_x + arm_len, uav_y + arm_len, uav_z + body_h/2],
    [uav_x - arm_len, uav_y + arm_len, uav_z + body_h/2],
    [uav_x + arm_len, uav_y - arm_len, uav_z + body_h/2],
    [uav_x - arm_len, uav_y - arm_len, uav_z + body_h/2]
]
...
for rx, ry, rz in rotor_positions:
    ...
    circle_x = rx + rotor_r * np.cos(theta)
    circle_y = ry + rotor_r * np.sin(theta)
...
# Step 4: Draw 3D cable and payload
phi = 22 * np.pi / 180
psi = 30 * np.pi / 180
L = 3.2
...
# Payload
payload_size = 0.4
payload_vertices = draw_box(ax, [payload_x, payload_y, payload_z],
                           payload_size, payload_size, payload_size,
                           color='#8B6F47', alpha=0.95)

# Payload label - positioned on the right side of the payload
...
# Cable length label - positioned on the right side of the cable
...
# Step 4.4: Draw projection lines for psi angle
ax.plot([uav_x, uav_x], [uav_y, uav_y], [uav_z, ground_z],
        color='#5B7C99', linestyle='--', linewidth=2, alpha=0.6, zorder=5)

ax.plot([payload_x, payload_x], [payload_y, payload_y], [payload_z, ground_z],
        color='#5B7C99', linestyle='--', linewidth=2, alpha=0.6, zorder=5)

ax.plot([uav_x, payload_x], [uav_y, payload_y], [ground_z, ground_z],
        color='#CD853F', linewidth=2.5, alpha=0.8, linestyle='-', zorder=6)

# Step 5: Draw swing angle phi
...
arc_r = 1.2
arc_n = 40
arc_theta = np.linspace(0, phi, arc_n)

arc_x = uav_x + arc_r * np.sin(arc_theta) * np.cos(psi)
arc_y = uav_y + arc_r * np.sin(arc_theta) * np.sin(psi)
arc_z = uav_z - arc_r * np.cos(arc_theta)
...
# Step 6: Draw yaw angle psi in ground plane
x_ref_len = 1.0
ax.plot([uav_x, uav_x + x_ref_len], [uav_y, uav_y], [ground_z, ground_z],
        color='#333333', linewidth=2.5, alpha=0.8, zorder=6)

psi_arc_r = 0.8
psi_arc_n = 35
psi_arc_theta = np.linspace(0, psi, psi_arc_n)
psi_arc_x = uav_x + psi_arc_r * np.cos(psi_arc_theta)
psi_arc_y = uav_y + psi_arc_r * np.sin(psi_arc_theta)
psi_arc_z = np.ones(psi_arc_n) * ground_z
ax.plot(psi_arc_x, psi_arc_y, psi_arc_z, color='#9370DB', linewidth=3, zorder=6)

mid_psi = psi_arc_n // 2
ax.text(psi_arc_x[mid_psi]+0.3, psi_arc_y[mid_psi]+0.3, ground_z+0.3,
        r'$\psi$ (yaw of swing)',
        fontsize=12, fontweight='bold', color='#333333',
        bbox=dict(boxstyle='round,pad=0.4', facecolor='white',
                  edgecolor='#9370DB', linewidth=2, alpha=0.95))

# Step 7: Draw 3D wind disturbance arrow
...
plt.title('Fig. 1 Three-dimensional physical configuration of the UAV slung-load system\n' +
          'with inertial frame, swing angles, and cable length',
