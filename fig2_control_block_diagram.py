...
# 设置中文字体
plt.rcParams['font.sans-serif'] = ['SimHei', 'Microsoft YaHei', 'WenQuanYi Zen Hei', 'WenQuanYi Micro Hei', 'DejaVu Sans']
...
# 系统边界虚线框
系统边界 = FancyBboxPatch((3.2, 2.2), 3.3, 6.7,
...
# 绘制箭头函数
def 绘制箭头(ax, x1, y1, x2, y2, 颜色='#4a5568', 宽度=2, 线型='solid', 连接方式="arc3"):
    arrow = FancyArrowPatch((x1, y1), (x2, y2),
                           arrowstyle='->',
                           color=颜色,
                           linewidth=宽度,
                           linestyle=线型 if 线型 != 'solid' else '-',
                           mutation_scale=20,
...
# 箭头1: 外部干扰 → 无人机 (干扰输入)
绘制箭头(ax, 2.5, 7.75, 3.5, 7.7, 颜色='#4a5568', 宽度=2)
...
# 箭头3: 无人机 → 缆绳 (悬挂连接)
绘制箭头(ax, 4.75, 6.8, 4.75, 5.8, 颜色='#4a5568', 宽度=2.5, 连接方式="arc3,rad=0")
ax.text(5.2, 6.3, '悬挂连接', fontsize=10, ha='left', color='#4a5568')

# 箭头4: 缆绳 → 悬挂负载 (力传递)
绘制箭头(ax, 4.75, 4.5, 4.75, 3.8, 颜色='#4a5568', 宽度=2.5, 连接方式="arc3,rad=0")
ax.text(5.2, 4.15, '力传递', fontsize=10, ha='left', color='#4a5568')

# 箭头5: 悬挂负载 → 缆绳 (摆动影响 - 虚线)
绘制箭头(ax, 4.5, 3.8, 4.5, 4.5, 颜色='#4a5568', 宽度=2, 线型='--', 连接方式="arc3,rad=0")
...
# 箭头9: 输入 → MPC控制器
绘制箭头(ax, 10.4, 7.5, 10.4, 6.4, 颜色='#4a5568', 宽度=2, 连接方式="arc3,rad=0")

# 箭头10: MPC控制器 → 输出
绘制箭头(ax, 10.4, 4.2, 10.4, 3.0, 颜色='#4a5568', 宽度=2, 连接方式="arc3,rad=0")

# 图例
图例_x = 0.5
图例_y = 1.5
ax.text(图例_x, 图例_y, '图例:', fontsize=12, fontweight='bold', color='#2d3748')

# 图例项1
ax.plot([图例_x, 图例_x+0.5], [图例_y-0.3, 图例_y-0.3],
        color='#4a5568', linewidth=2, marker='>', markersize=8, markerfacecolor='#4a5568')
ax.text(图例_x+0.7, 图例_y-0.3, '物理连接/力传递', fontsize=10, va='center', color='#4a5568')

# 图例项2
ax.plot([图例_x, 图例_x+0.5], [图例_y-0.6, 图例_y-0.6],
        color='#d97706', linewidth=2, marker='>', markersize=8, markerfacecolor='#d97706')
ax.text(图例_x+0.7, 图例_y-0.6, '状态反馈信号', fontsize=10, va='center', color='#4a5568')

# 图例项3
ax.plot([图例_x, 图例_x+0.5], [图例_y-0.9, 图例_y-0.9],
        color='#16a34a', linewidth=2, marker='>', markersize=8, markerfacecolor='#16a34a')
ax.text(图例_x+0.7, 图例_y-0.9, '控制信号输出', fontsize=10, va='center', color='#4a5568')

# 图例项4
ax.plot([图例_x, 图例_x+0.5], [图例_y-1.2, 图例_y-1.2],
        color='#4a5568', linewidth=2, linestyle='--', marker='>', markersize=8, markerfacecolor='#4a5568')
ax.text(图例_x+0.7, 图例_y-1.2, '间接影响', fontsize=10, va='center', color='#4a5568')
